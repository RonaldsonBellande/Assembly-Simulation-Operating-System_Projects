...
Status test_get_size_on_init_default_return_0(char* buffer,int length){
  ...
  if(my_string_get_size(hString) != 0){
    ...
    status = SUCCESS;
    strncpy(buffer, "test_get_size_on_init_returns_0\n",length);
  }
  my_string_destroy(&hString);
  return status;
}
...
Status test_get_capacity_on_init_default_return_0(char* buffer, int length){
  ...
  if(my_string_get_capacity(hString) != 0){
    ...
    strncpy(buffer, "test_get_capacity_on_init_default_return_0\n"
	    "Did not received 0 from get_capacity after init_default\n", length);
  }
  else{
    status = SUCCESS;
    strncpy(buffer, "test_get_capacity_on_init_return_0\n",length);
  }
  my_string_destroy(&hString);
  return status;
}
...
Status test_akveraga_my_string_at_returns_value(char* buffer, int length)
 {
 	Status status;
 	MY_STRING temp;
 	temp = my_string_init_c_string("temp");

 	if (my_string_at(temp, 2) != NULL)
 	{
 		status = SUCCESS;
 	}
 	else {
 		status = FAILURE;
 		strncpy(buffer, "test_akveraga_my_string_at_returns_value\n" 
 		"my_string_at returns NULL after accessing an element in the string", length); 
 	}
...
Status test_akveraga_my_string_compare_left_string_larger(char* buffer, int length)
{
  ...
  l = my_string_init_c_string("Left is Bigger");
  r = my_string_init_c_string("Than right");
  if (my_string_compare(l, r) >= 0){
 	  status = FAILURE;
 	  strncpy(buffer, "test_akveraga_my_string_compare_left_string_larger\n" 
 	  "my_string_compare returned that the right string was larger than or equal to left", length); 
